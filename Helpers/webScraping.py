...
class WebScraping:
    ...
    def __init__(self, dominio_base: str = "https://www.minjusticia.gov.co/normatividad-co/"):
        """
    ...
    def extract_links(self, url: str, listado_extensiones: List[str] = None) -> List[Dict]:
        ...
        try:
            ...
            # Si la página no tiene div.containerblanco, usamos todo el HTML
            ...
            if container_div:
                for link in container_div.find_all('a'):
                    ...
                    # Restringir al dominio base
                    if not full_url.startswith(self.dominio_base):
                        continue

                    # Verificar extensión tolerando parámetros (?x=1)
            ...
            print(f"Total links encontrados en {url}: {len(links)}")
            return links
